...
export const WorkflowListItem: React.FC<WorkflowListItemProps> = ({ workflow, onView, onEdit, onDelete }) => {
  ...
  return (
    ...
    >
      <div className="absolute top-4 right-4 flex items-center gap-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
        <button onClick={(e) => handleActionClick(e, onEdit)} className="p-1.5 bg-gray-100 dark:bg-gray-700/80 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors" title="Edit Workflow">
          <EditIcon />
        </button>
        <button onClick={(e) => handleActionClick(e, onDelete)} className="p-1.5 bg-gray-100 text-red-500 dark:bg-gray-700/80 dark:text-red-400 rounded-full hover:bg-red-100 dark:hover:bg-red-500/30 transition-colors" title="Delete Workflow">
          <TrashIcon />
        </button>
      </div>

      <div className="flex-1 space-y-3">
        <h3 className="font-bold text-gray-900 dark:text-gray-100 text-lg pr-16">{workflow.title}</h3>
